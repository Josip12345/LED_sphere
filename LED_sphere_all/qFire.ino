/**
 * ledsFireFast - A fire simulation for NeoPixel (and other?) leds
 ...
 *
 * Github: https://github.com/toggledbits/ledsFireFast
...
/* leds CONFIGURATION -- PLEASE SEE THE README (GITHUB LINK ABOVE) */

#define MAT_TYPE NEOPIXEL   /* leds LED type; see FastLED docs for others */
#define MAT_W   8          /* Size (columns) of entire leds */
#define MAT_H   16         /* and rows */
#if defined(ESP32) || defined(ESP8266)
#define MAT_PIN 3           /* Data for leds on D3 on ESP8266 */
#else
#define MAT_PIN 6           /* Data for leds on pin 6 for Arduino/other */
#endif
#undef  MAT_COL_MAJOR       /* define if leds is column-major (that is pixel 1 is in the same column as pixel 0) */
#undef MAT_TOP             /* define if leds 0,0 is in top row of display; undef if bottom */
#define MAT_LEFT            /* define if leds 0,0 is on left edge of display; undef if right */
#undef MAT_ZIGZAG          /* define if leds zig-zags ---> <--- ---> <---; undef if scanning ---> ---> ---> */

#define BRIGHT 100           /* brightness; min 0 - 255 max -- high brightness requires a hefty power supply! Start low! */
#define FPS 30              /* Refresh rate */
...
/* Display size; can be smaller than leds size, and if so, you can move the origin.
 * This allows you to have a small fire display on a large leds sharing the display
...
//CRGB leds[MAT_H * PANELS_H * MAT_W * PANELS_W];
uint8_t nflare = 0;
...
void Fire_setup() {
  //FastLED.addLeds<MAT_TYPE, MAT_PIN>(leds, MAT_W * MAT_H);
  //FastLED.setBrightness(BRIGHT); // Initial brightness
//  FastLED.clear();
//  FastLED.show();
...
//#ifdef SERIAL
    ...
    //Serial.print("ledsFireFast v"); Serial.println(VERSION);
...
//#endif
  ...
  for ( uint16_t i=0; i<cols; ++i ) {
    leds[pos(i,0)] = colors[NCOLORS-1];
    FastLED.show();
    delay(1000/FPS);
    leds[pos(i,0)] = 0;
  }
  for ( uint16_t i=0; i<rows; ++i ) {
    leds[pos(0,i)] = colors[NCOLORS-1];
    FastLED.show();
    delay(1000/FPS);
    leds[pos(0,i)] = 0;
  }
  ...
  for ( int i=NCOLORS-1; i>=0; --i ) {
    if ( y < rows ) {
      leds[pos(0,y)] = colors[i];
      leds[pos(cols-1,y++)] = colors[i];
    }
    else break;
  }
  FastLED.show();
